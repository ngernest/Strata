...
-------------------------------------------------------------------------
-- Safe, graph-time structural hash
-------------------------------------------------------------------------
...
def hashExprPtrCache {T : LExprParamsT} [Hashable T.TypeType] :
  ...
  | .const _ c => pure ⟨hashConst (hash c), by simp [hashExpr]⟩
  | .bvar _ i => pure ⟨hashBVar (hash i), by simp [hashExpr]⟩
  | .fvar _ n ty => pure ⟨hashFVar (hash n.name) (hashOptTy ty), by simp [hashExpr]⟩
  | .op _ o ty => pure ⟨hashOp (hash o.name) (hashOptTy ty), by simp [hashExpr]⟩
  | .app _ fn arg => do
    let rfn ← evalPtrCache fn (hashExprPtrCache fn)
    let rarg ← evalPtrCache arg (hashExprPtrCache arg)
    pure ⟨hashApp rfn.output rarg.output, by simp [hashExpr, rfn.h, rarg.h]⟩
  | .ite _ c t f => do
    let rc ← evalPtrCache c (hashExprPtrCache c)
    let rt ← evalPtrCache t (hashExprPtrCache t)
    let rf ← evalPtrCache f (hashExprPtrCache f)
    pure ⟨hashIte rc.output rt.output rf.output, by simp [hashExpr, rc.h, rt.h, rf.h]⟩
  | .eq _ e1 e2 => do
    let r1 ← evalPtrCache e1 (hashExprPtrCache e1)
    let r2 ← evalPtrCache e2 (hashExprPtrCache e2)
    pure ⟨hashEqExpr r1.output r2.output, by simp [hashExpr, r1.h, r2.h]⟩
  | .abs _ name ty body => do
    let rb ← evalPtrCache body (hashExprPtrCache body)
    pure ⟨hashAbs (hash name) (hashOptTy ty) rb.output, by simp [hashExpr, rb.h]⟩
  | .quant _ k name ty tr body => do
    ...
    pure ⟨hashQuantExpr (match k with | .all => 0 | .exist => 1)
          ...
          by cases k <;> simp [hashExpr, rtr.h, rb.h]⟩
