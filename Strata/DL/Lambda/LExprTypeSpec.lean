...
theorem RigidAnnotCompat.of_aliasEquiv {aliases : List TypeAlias}
  ...
  ⟨[], fun _ _ => by unfold LMonoTy.subst; simp [Subst.hasEmptyScopes, Map.isEmpty], by
    unfold LMonoTy.subst; simp [Subst.hasEmptyScopes, Map.isEmpty]; exact h⟩
...
@[expose] def Subst.polyKeysFresh {T : LExprParams} [DecidableEq T.IDMeta]
    (S : Subst) (Γ : TContext T.IDMeta) : Prop :=
...
theorem LTy.resolveAliases_preserves_stateSubstInfo {IDMeta : Type} [ToFormat IDMeta]
    (ty : LTy) (Env : TEnv IDMeta) (mty : LMonoTy) (Env' : TEnv IDMeta)
    (h : LTy.resolveAliases ty Env = .ok (mty, Env')) :
...
private theorem LMonoTy.freeVars_subst_single_mem
  ...
  · have hS_false : Subst.hasEmptyScopes [[(a, t)]] = false := by
    ...
    | .ftvar x =>
      ...
      by_cases hax : a = x
      · subst hax
        have h_find : Maps.find? [[(a, t)]] a = some t := by
...
private theorem typeBoundVar_absorbs
  ...
  · -- some bty_val
    ...
    have h_abs := LMonoTy_instantiateWithCheck_absorbs bty_mty C
    ...
    | cons x xs =>
      ...
      | inr h_subst_fvs =>
        ...
        have h_v_in_freshtvs : v ∈ freshtvs := by
          ...
          suffices h_val_ftvar : ∀ (vars : List TyIdentifier) (tvs : List TyIdentifier),
              mty_val ∈ Map.values (List.zip vars (tvs.map LMonoTy.ftvar)) →
              ∃ t ∈ tvs, mty_val = .ftvar t by
            simp at h_in_vals
            obtain ⟨t, h_t_mem, h_eq⟩ := h_val_ftvar (x :: xs) freshtvs h_in_vals
...
private theorem LTy_instantiateWithCheck_preserves_SubstFreshForGen
  ...
  elim_errs h  -- checkNoFutureGenVars
  simp at h; obtain ⟨_, h2⟩ := h; rw [← h2]
...
theorem typeBoundVar_preserves_invariant
  ...
  have h_gen_tyGen : genResult.snd.genEnv.genState.tyGen ≥ Env.genEnv.genState.tyGen := by
    elim_err h_gen
    rename_i _ _ h_genVar
    have h_gen_eq := Except.ok.inj h_gen; rw [← h_gen_eq]; simp
    exact _root_.Lambda.HasGen.genVar_tyGen_mono Env.genEnv _ _ h_genVar
  -- liftGenEnv preserves context
  have h_gen_ctx : genResult.snd.context = Env.context := by
    elim_err h_gen
    rename_i _ _ h_genVar
    have h_gen_eq := Except.ok.inj h_gen; rw [← h_gen_eq]; simp [TEnv.context]
...
omit [ToString T.IDMeta] [DecidableEq T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
...
theorem TEnv.pushEmptyContext_knownTypeVars
  ...
  simp only [TEnv.pushEmptyContext, TEnv.updateContext, TEnv.context, Maps.push,
    TContext.knownTypeVars, TContext.types.knownTypeVars, TContext.types.knownTypeVars.go,
    List.nil_append]
...
omit [ToString T.IDMeta] [DecidableEq T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
/-- `addInNewest` distributes over a `cons`: inserting `b :: rest` is the same as
    inserting `[b]` first and then `rest`. This lets us fold a multi-binding
    `addInNewestContext` into repeated single-binding insertions. -/
theorem Maps.addInNewest_cons_eq
    (ms : Maps T.Identifier LTy) (b : T.Identifier × LTy) (rest : Map T.Identifier LTy) :
    Maps.addInNewest ms (b :: rest) = Maps.addInNewest (Maps.addInNewest ms [b]) rest := by
  cases ms with
  | nil => rfl
  | cons mm tl =>
    simp only [Maps.addInNewest, Maps.newest, Maps.pop, Maps.push]
    congr 1
    exact List.append_cons mm b rest

omit [ToString T.IDMeta] [DecidableEq T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
/-- `addInNewestContext` distributes over a `cons` at the `TEnv` level. -/
theorem TEnv.addInNewestContext_cons_eq
    (Env : TEnv T.IDMeta) (b : T.Identifier × LTy) (rest : Map T.Identifier LTy) :
    Env.addInNewestContext (b :: rest) =
      (Env.addInNewestContext [b]).addInNewestContext rest := by
  simp only [TEnv.addInNewestContext, TEnv.updateContext, TEnv.context]
  congr 3
  exact Maps.addInNewest_cons_eq Env.genEnv.context.types b rest
...
theorem TEnvWF.of_addInNewestContext_singleton
    ...
    (h_fresh : ∀ v, v ∈ LMonoTy.freeVars mty →
      ∀ n, n ≥ Env.genEnv.genState.tyGen → v ≠ TState.tyPrefix ++ toString n) :
    TEnvWF (Env.addInNewestContext [(x, .forAll [] mty)]) := by
  have h_subst : (Env.addInNewestContext [(x, .forAll [] mty)]).stateSubstInfo =
      Env.stateSubstInfo := rfl
  have h_gen : (Env.addInNewestContext [(x, .forAll [] mty)]).genEnv.genState =
      Env.genEnv.genState := rfl
  have h_aliases : (Env.addInNewestContext [(x, .forAll [] mty)]).context.aliases =
...
omit [ToString T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
/-- `find?` is unaffected by inserting the empty map: `addInNewest` with `[]`
    leaves every scope's contents unchanged (a possibly-empty newest scope gains
    no entries). -/
theorem TEnv.addInNewestContext_nil_find?
    (Env : TEnv T.IDMeta) (y : T.Identifier) :
    (Env.addInNewestContext []).context.types.find? y = Env.context.types.find? y := by
  simp only [TEnv.addInNewestContext, TEnv.updateContext, TEnv.context]
  cases h : Env.genEnv.context.types with
  | nil => rfl
  | cons mm tl =>
    simp only [Maps.addInNewest, Maps.newest, Maps.pop, Maps.push]
    rw[Map.append_nil]
...
omit [ToString T.IDMeta] [DecidableEq T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
/-- `knownTypeVars` is unaffected by inserting the empty map. -/
theorem TEnv.addInNewestContext_nil_knownTypeVars
    (Env : TEnv T.IDMeta) :
    TContext.knownTypeVars (Env.addInNewestContext []).context =
      TContext.knownTypeVars Env.context := by
  simp only [TEnv.addInNewestContext, TEnv.updateContext, TEnv.context, TContext.knownTypeVars]
  cases h : Env.genEnv.context.types with
  | nil => simp only [Maps.addInNewest, Maps.newest, Maps.pop, Maps.push, Map.append_nil,
      TContext.types.knownTypeVars, TContext.types.knownTypeVars.go, List.append_nil]
  | cons mm tl => simp only [Maps.addInNewest, Maps.newest, Maps.pop, Maps.push, Map.append_nil]

omit [ToString T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
/-- Inserting the empty map preserves `TEnvWF`: `find?`, `knownTypeVars`,
    `aliases`, `genState`, and the substitution are all unchanged. -/
theorem TEnvWF.of_addInNewestContext_nil
    (Env : TEnv T.IDMeta) (h_envwf : TEnvWF Env) :
    TEnvWF (Env.addInNewestContext []) := by
  have h_subst : (Env.addInNewestContext []).stateSubstInfo = Env.stateSubstInfo := rfl
  have h_gen : (Env.addInNewestContext []).genEnv.genState = Env.genEnv.genState := rfl
  have h_aliases : (Env.addInNewestContext []).context.aliases = Env.context.aliases := rfl
  refine {
    aliasesWF := ?_
    substFreshForGen := ?_
    ctxFreshForGen := ?_
  ...
  · rw [h_subst, h_gen]; exact h_envwf.substFreshForGen
  · intro v hv n hn
    rw [TEnv.addInNewestContext_nil_knownTypeVars] at hv
    rw [h_gen] at hn
    exact h_envwf.ctxFreshForGen v hv n hn
  · intro y ty h_find
    rw [TEnv.addInNewestContext_nil_find?] at h_find
    exact h_envwf.boundVarsNodup y ty h_find
  · intro y ty h_find v hv n hn
    rw [TEnv.addInNewestContext_nil_find?] at h_find
    rw [h_gen] at hn
...
omit [ToString T.IDMeta] [HasGen T.IDMeta] [ToFormat T.IDMeta] [ToFormat (LFunc T)] [ToFormat T.Metadata] in
/-- Inserting a list of monomorphic bindings `(id, forAll [] mty)` whose free type
    variables are all gen-fresh preserves `TEnvWF`. Proved by induction over the
    list, distributing each `cons` into a single-binding insertion
    (`TEnvWF.of_addInNewestContext_singleton`). The generator state is unchanged
    by each insertion, so the freshness hypothesis carries to the tail. -/
theorem TEnvWF.of_addInNewestContext_mono
    ...
    (h_fresh : ∀ p ∈ pairs, ∀ v, v ∈ LMonoTy.freeVars p.2 →
      ∀ n, n ≥ Env.genEnv.genState.tyGen → v ≠ TState.tyPrefix ++ toString n) :
    TEnvWF (Env.addInNewestContext (pairs.map (fun p => (p.1, .forAll [] p.2)))) := by
  induction pairs generalizing Env with
  | nil =>
    show TEnvWF (Env.addInNewestContext [])
    exact TEnvWF.of_addInNewestContext_nil Env h_envwf
  | cons p ps ih =>
    rw [List.map_cons, TEnv.addInNewestContext_cons_eq]
    -- Insert the head binding first; gen state is unchanged.
    have h_head_fresh : ∀ v, v ∈ LMonoTy.freeVars p.2 →
        ∀ n, n ≥ Env.genEnv.genState.tyGen → v ≠ TState.tyPrefix ++ toString n :=
      h_fresh p List.mem_cons_self
    have h_envwf1 : TEnvWF (Env.addInNewestContext [(p.1, .forAll [] p.2)]) :=
      TEnvWF.of_addInNewestContext_singleton Env p.1 p.2 h_envwf h_head_fresh
    have h_gen1 : (Env.addInNewestContext [(p.1, .forAll [] p.2)]).genEnv.genState =
        Env.genEnv.genState := rfl
    have h_tail_fresh : ∀ q ∈ ps, ∀ v, v ∈ LMonoTy.freeVars q.2 →
        ∀ n, n ≥ (Env.addInNewestContext [(p.1, .forAll [] p.2)]).genEnv.genState.tyGen →
          v ≠ TState.tyPrefix ++ toString n := by
      intro q hq v hv n hn
      rw [h_gen1] at hn
      exact h_fresh q (List.mem_cons_of_mem p hq) v hv n hn
    exact ih (Env.addInNewestContext [(p.1, .forAll [] p.2)]) h_envwf1 h_tail_fresh
...
theorem TContext.AliasesResolved.of_addInNewestContext
    (Env : TEnv T.IDMeta) (map : Map T.Identifier LTy)
...
private theorem typeBoundVar_erase_context
  ...
  -- Step 4: remove_addInNewest_fresh cancels the add
  ...
  have h_types : Env1.context.types =
    ...
    suffices h_ctx_add : ∀ Env1, typeBoundVar C Env bty = .ok (xv, xty, Env1) →
        Env1.context.types = Env.context.types.addInNewest [(xv, .forAll [] xty)] ∧
        Env1.context.aliases = Env.context.aliases by
      intro h_ctx h_nonempty h_fresh_xv; exact h_ctx_add Env1 h
  ...
  have h_erase_types : (Env2.eraseFromContext xv).context.types = Env1.context.types.remove xv := by
    show (TEnv.eraseFromContext Env2 xv).context.types = _
    unfold TEnv.eraseFromContext TEnv.updateContext TEnv.context
    simp; exact congrArg (Maps.remove · xv) (congrArg TContext.types h_ctx)
  ...
  have h_cancel : Env1.context.types.remove xv = Env.context.types := by
    rw [h_ty]
    cases h_types_ne : Env.context.types with
    | nil => exact absurd h_types_ne h_nonempty
    | cons m rest =>
      exact Maps.remove_addInNewest_fresh xv _ (fun s hs => h_fresh_xv s (h_types_ne ▸ hs))
...
private theorem resolveAux_properties_aux :
  ...
  | .app m e1 e2 =>
    ...
    · -- SubstFreshForGen (Maps.remove preserves freshness)
      ...
      exact h_sf4 v (by
        cases hv with
        | inl h_key => exact Or.inl (Maps.keys_remove_subset _ _ _ h_key)
        | inr h_fv =>
          exact Or.inr (by
  ...
  | .abs m _ bty body =>
    ...
    · simp [LMonoTy.freeVars, LMonoTys.freeVars, List.mem_append] at hv_ty
      ...
      · -- v from xty_val: gen-fresh from typeBoundVar
        ...
        have h_gen_tyGen : genResult.snd.genEnv.genState.tyGen ≥ Env.genEnv.genState.tyGen := by
          elim_err h_gen
          rename_i _ _ h_gv; have h_gen_eq := Except.ok.inj h_gen; rw [← h_gen_eq]; simp
          exact HasGen.genVar_tyGen_mono Env.genEnv _ _ h_gv
        have h_gen_ctx : genResult.snd.context = Env.context := by
          elim_err h_gen
          rename_i _ _ h_gv; have h_gen_eq := Except.ok.inj h_gen; rw [← h_gen_eq]; simp [TEnv.context]
...
private theorem subst_openVars_comm
  ...
  · -- S is empty: subst S is identity
    ...
    have h_substLogic : LMonoTys.substLogic S vals = vals := by
      induction vals with
...
private theorem subst_single_scope_eq_openVars
  ...
  | nil =>
    ...
    have h_vals_nil : vals = [] := by simpa using h_len.symm
    subst h_vals_nil
...
private theorem subst_single_scope_eq_openVarsList
  ...
  | nil =>
    ...
    have h_substLogic_nil : LMonoTys.substLogic [([] : Map TyIdentifier LMonoTy)] bodies = bodies := by
...
private theorem keys_go_subset_keys (S : Subst) (xs : List TyIdentifier)
  ...
  | cons x rest ih =>
    ...
    exact Maps.keys_remove_subset S x a (ih (Maps.remove S x) h)
...
private theorem keys_go_not_mem_xs (S : Subst) (xs : List TyIdentifier)
  ...
  | cons x rest ih =>
    ...
    · -- a = x
      have h_a_key := keys_go_subset_keys (Maps.remove S a) rest a h
      exact (Maps.keys_remove_self_not_mem S a h_a_key).elim
    · exact ih (Maps.remove S x) h h_rest

/-- If all keys of `S` that are NOT in `xs` are also not free vars of `mty`,
    then `subst (go xs S) mty = mty`. This follows because `go xs S` removes
      ...
      | tcons name args ih_args =>
        ...
        have h_per_arg : ∀ a, a ∈ args →
          ...
          fun a ha => ih_args a ha (fun v hv hni => by
            ...
            have h_mem_fv : ∀ (l : LMonoTys), a ∈ l → v ∈ LMonoTy.freeVars a → v ∈ LMonoTys.freeVars l := by
        ...
        suffices h_list_gen : ∀ (l : LMonoTys),
            (∀ a, a ∈ l →
              LMonoTy.subst S (LMonoTy.subst [List.zip (id :: ids') (List.map LMonoTy.ftvar (ftv :: ftvs'))] a) =
              LMonoTy.subst [List.zip (id :: ids') (List.map (fun v => LMonoTy.subst S (.ftvar v)) (ftv :: ftvs'))] a) →
...
private theorem tconsAlias_expand_eq
  ...
  elim_err h_tcons  -- instantiateEnv failed
    ...
    | cons a tl =>
      ...
      | cons b rest =>
        ...
        have h_pat_wf : ∀ tv, tv ∈ LMonoTy.freeVars (.tcons name (alias.typeArgs.map .ftvar)) → tv ∈ alias.typeArgs := by
          ...
          have h_ftvar_mem : ∀ (ids : List TyIdentifier), tv ∈ LMonoTys.freeVars (ids.map .ftvar) → tv ∈ ids := by
        ...
        have h_extract : ∀ (S : Subst) (xs : LMonoTys),
          ...
          · have hS'_false := show Subst.hasEmptyScopes S' = false by
              revert hS'; cases Subst.hasEmptyScopes S' <;> simp
            simp [LMonoTy.subst, hS'_false]
  ...
  | tabs Γ m name x x_ty e e_ty o h_fresh hx he h_body h_o ih_body =>
    ...
    · -- TContextAliasEquiv at the extended contexts.
      ...
      · -- shadowed key: both sides hold `x_ty`; use x_ty's mono shape + refl.
        subst h_xy
        rw [Maps.find?_insert_self] at h_find_y
        ...
        rw [h_xty, Maps.find?_insert_self]
      · -- other key: reduce to the Γ/Γ' equivalence.
        rw [Maps.find?_insert_ne _ _ _ _ h_xy] at h_find_y
        obtain ⟨mty, mty', h_ty_eq, h_find', h_ae⟩ := h_ctx y ty_y h_find_y
        refine ⟨mty, mty', h_ty_eq, ?_, h_ae⟩
        rw [Maps.find?_insert_ne _ _ _ _ h_xy]; exact h_find'
    · -- per-key freshness reflection at the extended contexts.
      ...
      · -- shadowed key: both sides hold `x_ty`; reflection is trivial.
        ...
        obtain ⟨h_yeq⟩ : ty_y = x_ty ∧ True := ⟨by injection h_find_y with h; exact h.symm, trivial⟩
        subst h_yeq
        exact ⟨ty_y, Maps.find?_insert_self _ _ _, h_mem⟩
      · -- other key: reduce to the Γ/Γ' reflection.
        rw [Maps.find?_insert_ne _ _ _ _ h_xy] at h_find_y
        obtain ⟨ty0, h_find0, h_mem0⟩ := h_fr y ty_y h_find_y a h_mem
        exact ⟨ty0, by rw [Maps.find?_insert_ne _ _ _ _ h_xy]; exact h_find0, h_mem0⟩
    · rw [h_al]; exact h_o
  | tapp Γ m e1 e2 t1 t2 h1 h2 h_e1 h_e2 ih_e1 ih_e2 =>
    intro Γ' h_al h_ctx h_fr
...
theorem LTy_instantiate_eq_openFull (ty : LTy) (Env Env' : TGenEnv T.IDMeta)
  ...
  | .forAll [] body =>
    ...
    simp only [LTy.openFull, LTy.boundVars, LTy.toMonoTypeUnsafe, List.zip_nil_left]
    rw [LMonoTy.subst_emptyS (by simp [Subst.hasEmptyScopes, Map.isEmpty]), h.1]
  | .forAll (x :: xs) body =>
...
theorem subst_cons_eq' (S : Subst) (hd : LMonoTy) (tl : LMonoTys) :
  ...
  rw [LMonoTys.subst_eq_substLogic, LMonoTys.subst_eq_substLogic]
  by_cases hS : Subst.hasEmptyScopes S
  · rw [LMonoTys.substLogic_emptyS hS, LMonoTys.substLogic_emptyS hS, LMonoTy.subst_emptyS hS]
  · have hS_ne : Subst.hasEmptyScopes S = false := by
      revert hS; cases Subst.hasEmptyScopes S <;> simp
    simp only [LMonoTys.substLogic, hS_ne, Bool.false_eq_true, if_false]
...
theorem subst_openVars_comm' (σ : Subst)
    ...
    LMonoTy.subst σ (LMonoTy.openVars vars vals body) =
      LMonoTy.openVars vars (LMonoTys.substLogic σ vals) body :=
  subst_openVars_comm σ vars vals body h_wf h_len
...
theorem tconsAliasSimple_subst_comm (σ : Subst) (name : String) (args : LMonoTys)
  ...
  have h_len : (LMonoTys.subst σ args).length = args.length := by
    rw [LMonoTys.subst_eq_substLogic, LMonoTys.substLogic_length]
...
/-- Round-trip on a single `.ftvar`: for `x ∈ ids`, the forward renaming
    `σ = zip ids (map ftvar freshtvs)` sends `x` to some fresh `ftv ∈ freshtvs`,
    and the inverse renaming `ρ₀ = zip freshtvs (map ftvar ids)` sends `ftv` back
    to `x`. Requires `freshtvs.Nodup` so the inverse lookup is unambiguous. -/
theorem find?_zip_ftvar_roundtrip
    ...
    ∃ ftv, ftv ∈ freshtvs ∧
      Map.find? (List.zip ids (List.map LMonoTy.ftvar freshtvs)) x = some (.ftvar ftv) ∧
      Map.find? (List.zip freshtvs (List.map LMonoTy.ftvar ids)) ftv = some (.ftvar x) := by
  induction ids generalizing freshtvs with
  ...
  | cons id ids' ih =>
    ...
    | [] => simp at h_len
    | f :: fs' =>
      have h_len' : ids'.length = fs'.length := by simpa using h_len
      have h_nodup' : fs'.Nodup := (List.nodup_cons.mp h_nodup).2
      have h_f_notin : f ∉ fs' := (List.nodup_cons.mp h_nodup).1
      by_cases h_x_id : x = id
      · -- x = id : maps to head fresh var f
        subst h_x_id
        refine ⟨f, List.mem_cons_self, ?_, ?_⟩
        · simp only [List.map, List.zip, List.zipWith, Map.find?, ↓reduceIte]
        · simp only [List.map, List.zip, List.zipWith, Map.find?, ↓reduceIte]
      · -- x ≠ id : recurse
        have h_x_ids' : x ∈ ids' := by
          cases List.mem_cons.mp h_mem with
          | inl h_eq => exact absurd h_eq h_x_id
          | inr h => exact h
        obtain ⟨ftv, h_ftv_mem, h_fwd, h_bwd⟩ := ih fs' h_len' h_nodup' h_x_ids'
        have h_id_ne : ¬ (id = x) := fun h => h_x_id h.symm
        have h_f_ne_ftv : ¬ (f = ftv) := fun h => h_f_notin (h ▸ h_ftv_mem)
        refine ⟨ftv, List.mem_cons_of_mem f h_ftv_mem, ?_, ?_⟩
        · simp only [List.map, List.zip, List.zipWith, Map.find?, if_neg h_id_ne]
          exact h_fwd
        · simp only [List.map, List.zip, List.zipWith, Map.find?, if_neg h_f_ne_ftv]
          exact h_bwd
...
theorem subst_rename_inverse
    ...
    LMonoTy.subst [List.zip freshtvs (List.map LMonoTy.ftvar ids)]
      (LMonoTy.subst [List.zip ids (List.map LMonoTy.ftvar freshtvs)] body) = body := by
  ...
  suffices H : ∀ b : LMonoTy, (∀ v, v ∈ LMonoTy.freeVars b → v ∈ ids) →
      LMonoTy.subst [List.zip freshtvs (List.map LMonoTy.ftvar ids)]
        (LMonoTy.subst [List.zip ids (List.map LMonoTy.ftvar freshtvs)] b) = b from
    H body h_closed
  intro b
  induction b with
  | ftvar x =>
    ...
    -- Forward scope is non-empty because the lookup succeeds.
    have h_ne_fwd : Subst.hasEmptyScopes [List.zip ids (List.map LMonoTy.ftvar freshtvs)] = false := by
      cases h_z : List.zip ids (List.map LMonoTy.ftvar freshtvs) with
      | nil => rw [h_z] at h_fwd; simp [Map.find?] at h_fwd
      | cons hd tl => simp [Subst.hasEmptyScopes, Map.isEmpty]
    have h_ne_bwd : Subst.hasEmptyScopes [List.zip freshtvs (List.map LMonoTy.ftvar ids)] = false := by
      cases h_z : List.zip freshtvs (List.map LMonoTy.ftvar ids) with
      | nil => rw [h_z] at h_bwd; simp [Map.find?] at h_bwd
      | cons hd tl => simp [Subst.hasEmptyScopes, Map.isEmpty]
    have h_step1 : LMonoTy.subst [List.zip ids (List.map LMonoTy.ftvar freshtvs)] (.ftvar x)
        = .ftvar ftv :=
      LMonoTy.subst_ftvar_eq _ x (.ftvar ftv) h_ne_fwd
        (by simp only [Maps.find?, h_fwd])
    rw [h_step1]
    exact LMonoTy.subst_ftvar_eq _ ftv (.ftvar x) h_ne_bwd (by simp only [Maps.find?, h_bwd])
  | bitvec n =>
    intro _
  ...
  | tcons name args ih =>
    ...
    have h_list : ∀ (l : LMonoTys),
        (∀ ty ∈ l, LMonoTy.subst [List.zip freshtvs (List.map LMonoTy.ftvar ids)]
          (LMonoTy.subst [List.zip ids (List.map LMonoTy.ftvar freshtvs)] ty) = ty) →
        LMonoTys.subst [List.zip freshtvs (List.map LMonoTy.ftvar ids)]
          (LMonoTys.subst [List.zip ids (List.map LMonoTy.ftvar freshtvs)] l) = l := by
      intro l
      induction l with
...
/-- Free variables of the inverse renaming `[zip l1 (map ftvar l2)]` are exactly `l2`
    (when `l1.length = l2.length`, so the zip does not truncate `l2`). -/
theorem freeVars_zip_map_ftvar (l1 l2 : List TyIdentifier) (h : l1.length = l2.length) :
    Subst.freeVars [List.zip l1 (List.map LMonoTy.ftvar l2)] = l2 := by
  simp only [Subst.freeVars, Maps.values, List.append_nil, List.zip]
  rw [Map.values_zipWith_eq_take, List.take_of_length_le (by simp [h])]
  exact flatMap_freeVars_ftvar l2

/-- Keys of the renaming `[zip l1 (map ftvar l2)]` are exactly `l1` when lengths match. -/
theorem keys_zip_map_ftvar (l1 l2 : List TyIdentifier) (h : l1.length = l2.length) :
    Map.keys (List.zip l1 (List.map LMonoTy.ftvar l2)) = l1 := by
  rw [Map.keys_eq_map_fst, List.map_fst_zip]; simp [h]

/-- Values of the renaming `[zip l1 (map ftvar l2)]` are exactly `map ftvar l2`
    when lengths match. -/
theorem values_zip_map_ftvar (l1 l2 : List TyIdentifier) (h : l1.length = l2.length) :
    Map.values (List.zip l1 (List.map LMonoTy.ftvar l2)) = List.map LMonoTy.ftvar l2 := by
  simp only [List.zip]
  rw [Map.values_zipWith_eq_take, List.take_of_length_le (by simp [h])]

/-- Any single-scope zip renaming `[zip ids (map ftvar freshtvs)]` sends every
...
theorem subst_zip_ftvar_renaming (ids freshtvs : List TyIdentifier) (y : TyIdentifier) :
    ∃ w, LMonoTy.subst [List.zip ids (List.map LMonoTy.ftvar freshtvs)] (.ftvar y) = .ftvar w := by
  rw [LMonoTy.subst_unfold]
  simp only [Maps.find?]
  cases h_f : Map.find? (List.zip ids (List.map LMonoTy.ftvar freshtvs)) y with
  | none => exact ⟨y, rfl⟩
  | some t =>
    have h_tval : t ∈ Map.values (List.zip ids (List.map LMonoTy.ftvar freshtvs)) :=
      Map.find?_mem_values _ h_f
...
theorem LTy_instantiateWithCheck_inverse
  ...
  | forAll xs body =>
    ...
    | nil =>
      ...
      have h_subst_id : LMonoTy.subst [([] : SubstOne)] mty_ra = mty_ra := by
        apply LMonoTy.subst_emptyS
        simp [Subst.hasEmptyScopes, Map.isEmpty]
      refine ⟨[], Env_ra, ?_, ?_, ?_, ?_, ?_, ?_⟩
      · -- SubstWF [[]]
        exact SubstWF_of_empty_empty
      · -- resolveAliases (toMonoTypeUnsafe ty) Env = .ok (subst [[]] mty_ra, Env_ra)
        show LMonoTy.resolveAliases (LTy.toMonoTypeUnsafe (LTy.forAll [] body)) Env
          = .ok (LMonoTy.subst [[]] mty_ra, Env_ra)
        simp only [LTy.toMonoTypeUnsafe, h_subst_id]
        exact h_ra
      · -- keys [] = [] : vacuous
        intro x hx; simp [Map.keys] at hx
      · -- freeVars mty_ra → keys [] : vacuous since freeVars mty_ra = []
        intro v hv; rw [h_mtyra_closed] at hv; exact absurd hv (by simp)
      · -- Subst.freeVars [[]] = [] : vacuous
        intro v hv; simp [Subst.freeVars, Maps.values, Map.values] at hv
      · -- Map.values [] = [] : vacuous
        intro mty hmty; simp [Map.values] at hmty
    | cons x rest =>
      ...
      refine ⟨List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest)), Env,
        ?_, ?_, ?_, ?_, ?_, ?_⟩
      · -- SubstWF: keys = freshtvs, freeVars = (x::rest); disjoint by h_disjoint.
        show SubstWF [List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest))] = true
        unfold SubstWF
        rw [show Maps.keys [List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest))]
              = Map.keys (List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest))) from by
              simp [Maps.keys]]
        rw [keys_zip_map_ftvar _ _ h_len']
        rw [List.all_eq_true]
        intro w hw
        rw [freeVars_zip_map_ftvar _ _ h_len']
        simp only [decide_eq_true_eq]
        exact h_disjoint w hw
      · -- resolveAliases commutes with the renaming, and the renaming inverts instantiation.
        show LMonoTy.resolveAliases (LTy.toMonoTypeUnsafe (LTy.forAll (x :: rest) body)) Env
          = .ok (LMonoTy.subst [List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest))] mty_ra, Env)
        ...
        have h_ra' : LMonoTy.resolveAliases
            (LMonoTy.subst [List.zip (x :: rest) (List.map LMonoTy.ftvar freshtvs)] body)
        ...
        -- aliases of the literal env and Env coincide.
        ...
        have h_comm2 := LMonoTy_resolveAliases_subst_comm
          [List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest))]
        ...
        have h_inv : LMonoTy.subst [List.zip freshtvs (List.map LMonoTy.ftvar (x :: rest))]
            (LMonoTy.subst [List.zip (x :: rest) (List.map LMonoTy.ftvar freshtvs)] body) = body :=
      ...
      · -- keys = freshtvs, all fresh in Env.context via genTyVars_allFresh.
        intro w hw
        rw [keys_zip_map_ftvar _ _ h_len'] at hw
        exact TGenEnv.genTyVars_allFresh _ _ _ _ h_gen w hw
      · -- freeVars mty_ra ⊆ keys = freshtvs.
        intro v hv
        rw [keys_zip_map_ftvar _ _ h_len']
        -- mty_ra = resolveAliases of (subst σ body); freeVars ⊆ freeVars(subst σ body) ⊆ freshtvs.
        have h_ra_env : LMonoTy.resolveAliases
            (LMonoTy.subst [List.zip (x :: rest) (List.map (fun tv => LMonoTy.ftvar tv) freshtvs)] body)
            ({genEnv := genEnvg, stateSubstInfo := Env.stateSubstInfo} : TEnv T.IDMeta)
            = .ok (mty_ra, Env_ra) := h_ra
        have h_fv_sub := LMonoTy_resolveAliases_freeVars_subset _ _ _ _ h_ra_env (by
          simp only [TEnv.context]; rw [h_gen_ctx]; exact h_aw)
        have h_v_body := h_fv_sub v hv
        exact LMonoTy.freeVars_subst_closed (x :: rest) freshtvs h_len.symm body h_body_closed v h_v_body
      · -- Subst.freeVars [ρ₀] = (x::rest) = boundVars.
        intro v hv
        rw [freeVars_zip_map_ftvar _ _ h_len'] at hv
        simp only [LTy.boundVars]; exact hv
      · -- values = map ftvar (x::rest): every value is a ftvar.
        intro mty hmty
        rw [values_zip_map_ftvar _ _ h_len'] at hmty
        obtain ⟨w, _, h_eq⟩ := List.mem_map.mp hmty
        exact ⟨w, h_eq.symm⟩

/-- The concrete σ produced by composing an inner AnnotCompat substitution with
    an outer S. Maps `v ↦ subst S (subst [σ] (ftvar v))` for each `v ∈ freeVars ann`. -/
def AnnotCompat_subst_witness (ann : LMonoTy) (σ : Map TyIdentifier LMonoTy) (S : Subst) :
    Map TyIdentifier LMonoTy :=
  (LMonoTy.freeVars ann).map (fun v => (v, LMonoTy.subst S (LMonoTy.subst [σ] (.ftvar v))))

/-- The concrete witness satisfies `AliasEquiv aliases (subst [σ'] ann) (subst S xty)`. -/
theorem AnnotCompat_subst_witness_spec {aliases : List TypeAlias} {ann xty : LMonoTy}
    (σ : Map TyIdentifier LMonoTy) (S : Subst)
    (h_ae : AliasEquiv aliases (LMonoTy.subst [σ] ann) xty)
    (h_aw : ∀ alias, alias ∈ aliases → TypeAlias.WF alias) :
    AliasEquiv aliases (LMonoTy.subst [AnnotCompat_subst_witness ann σ S] ann)
      (LMonoTy.subst S xty) := by
  ...
  suffices h_eq : LMonoTy.subst [AnnotCompat_subst_witness ann σ S] ann =
      LMonoTy.subst S (LMonoTy.subst [σ] ann) by
    rw [h_eq]; exact h_ae_S
  have h_find : ∀ v, v ∈ LMonoTy.freeVars ann →
      Maps.find? [AnnotCompat_subst_witness ann σ S] v = some (g v) := by
    intro v hv; unfold AnnotCompat_subst_witness Maps.find?; rw [Map.find?_of_map_self _ g v hv]
  suffices h_subst_gen : ∀ (mty : LMonoTy),
      (∀ v, v ∈ LMonoTy.freeVars mty → v ∈ LMonoTy.freeVars ann) →
      LMonoTy.subst [AnnotCompat_subst_witness ann σ S] mty =
        LMonoTy.subst S (LMonoTy.subst [σ] mty) from
    h_subst_gen ann (fun v hv => hv)
  intro mty h_sub
  let σ' := AnnotCompat_subst_witness ann σ S
  ...
  · have h_no_fv_ann : LMonoTy.freeVars ann = [] := by
      ...
      | cons v vs =>
        ...
        simp only [σ', AnnotCompat_subst_witness, h_fv, Subst.hasEmptyScopes, List.map] at hσ'_e
...
/-- Corollary: `AnnotCompat` is preserved under substitution. -/
theorem AnnotCompat_subst {aliases : List TypeAlias} {ann xty : LMonoTy}
  ...
  obtain ⟨σ, h_ae⟩ := h
  exact ⟨AnnotCompat_subst_witness ann σ S, AnnotCompat_subst_witness_spec σ S h_ae h_aw⟩

/-- `RigidAnnotCompat` is preserved under substitution when `S` is identity on
    rigid vars. Uses the concrete `AnnotCompat_subst_witness`. -/
theorem RigidAnnotCompat_subst {aliases : List TypeAlias} {rigidVars : List TyIdentifier}
  ...
  refine ⟨AnnotCompat_subst_witness ann σ S, fun v hv => ?_,
    AnnotCompat_subst_witness_spec σ S h_ae h_aw⟩
  have h_gv : LMonoTy.subst S (LMonoTy.subst [σ] (.ftvar v)) = .ftvar v := by
    rw [h_σ_rigid v hv, h_rigid v hv]
  show LMonoTy.subst [AnnotCompat_subst_witness ann σ S] (.ftvar v) = .ftvar v
  by_cases h_mem : v ∈ LMonoTy.freeVars ann
  · have h_find : Maps.find? [AnnotCompat_subst_witness ann σ S] v = some (.ftvar v) := by
      unfold AnnotCompat_subst_witness Maps.find?
      rw [Map.find?_of_map_self _ _ v h_mem]; exact congrArg some h_gv
    by_cases hσ'_e : Subst.hasEmptyScopes [AnnotCompat_subst_witness ann σ S]
    · exact LMonoTy.subst_emptyS hσ'_e
    · exact LMonoTy.subst_ftvar_eq _ v (.ftvar v) (Bool.eq_false_iff.mpr hσ'_e) h_find
  · by_cases hσ'_e : Subst.hasEmptyScopes [AnnotCompat_subst_witness ann σ S]
    · exact LMonoTy.subst_emptyS hσ'_e
    · have hσ'_ne := Bool.eq_false_iff.mpr hσ'_e
      have h_keys : Map.keys (AnnotCompat_subst_witness ann σ S) = LMonoTy.freeVars ann := by
        unfold AnnotCompat_subst_witness; rw[Map.keys_eq_map_fst];
        simp [List.map_map]
        apply List.map_id'' ; simp
      have h_none : Map.find? (AnnotCompat_subst_witness ann σ S) v = none :=
        Map.find?_none_of_not_mem_keys' _ v (h_keys ▸ h_mem)
      simp [LMonoTy.subst, hσ'_ne, Maps.find?, h_none]
...
private theorem Subst.freeVars_remove_subset (S : Subst) (x : TyIdentifier) :
    ∀ a, a ∈ Subst.freeVars (Maps.remove S x) → a ∈ Subst.freeVars S := by
  intro a ha; simp [Subst.freeVars] at ha ⊢
  obtain ⟨mty, h_val, h_fv⟩ := ha
  exact ⟨mty, Maps.values_remove_subset S x mty h_val, h_fv⟩

private theorem SubstWF_remove (S : Subst) (x : TyIdentifier) (h_wf : SubstWF S) :
    SubstWF (Maps.remove S x) := by
  simp [SubstWF] at h_wf ⊢; intro k hk hk_fv
  exact h_wf k (Maps.keys_remove_subset S x k hk) (Subst.freeVars_remove_subset S x k hk_fv)
...
private theorem SubstWF_go (S : Subst) (xs : List TyIdentifier) (h_wf : SubstWF S) :
  ...
  | cons x rest ih =>
    simp [LTy.subst.go]
    exact ih (Maps.remove S x) (SubstWF_remove S x h_wf)
...
private theorem keys_go_mem (S : Subst) (xs : List TyIdentifier) (a : TyIdentifier)
  ...
  | cons x rest ih =>
    simp [LTy.subst.go]
    apply ih (Maps.remove S x)
    · exact Maps.keys_remove_mem_of_ne h_key
...
/-- Reusable induction principle for `resolveAux`. Handles strong induction on
    ...
    `types ≠ []` through the chain of environments. -/
theorem resolveAux_ind
    ...
    (h_app : ∀ m e1 e2 et C Env Env'
      ...
      Env'.stateSubstInfo.subst = Maps.remove substInfo.subst fresh_name →
      Subst.absorbs (Maps.remove substInfo.subst fresh_name) Env1.stateSubstInfo.subst →
      Subst.absorbs (Maps.remove substInfo.subst fresh_name) Env2.stateSubstInfo.subst →
      ...
      TEnvWF Env → Env.context.types ≠ [] → FactoryWF C.functions →
      TEnvWF Env1 → Env1.context = Env.context →
      TEnvWF Env2 → Env2.context = Env.context →
    ...
    (h_quant : ∀ m qk name bty triggers body et C Env Env'
      ...
      TEnvWF Env1 → Env1.context.types ≠ [] →
      Env1.context.aliases = Env.context.aliases →
      TEnvWF Env2 → Env2.context = Env1.context →
      P (LExpr.varOpen 0 (xv, some xty) body) et_body C Env1 Env2 →
    ...
    (h_ite : ∀ m c th el et C Env Env'
      ...
      TEnvWF Env1 → Env1.context = Env.context →
      TEnvWF Env2 → Env2.context = Env.context →
      TEnvWF Env3 → Env3.context = Env.context →
  ...
  have h_main : ∀ (n : Nat) (e : LExpr T.mono), e.sizeOf = n →
    ...
    | .app m e1 e2 =>
      ...
      have h_ne1 : Env1.context.types ≠ [] := h_ctx1 ▸ h_ne'
      ...
      have h_ctx2 : Env2.context = Env.context := h_props2.context ▸ h_ctx1
      have h_envwf2 := TEnvWF.of_resolveAux e2 e2t C Env1 Env2 h2 h_envwf1 h_ne1 h_fwf' h_props2.context
      -- genTyVar facts and freshness/absorption derivations (S-independent)
      ...
      have h_fresh_e1 : Maps.find? Env1.stateSubstInfo.subst fresh_name = none ∧
          (∀ a t, Maps.find? Env1.stateSubstInfo.subst a = some t →
      ...
      have h_fresh_e2 : Maps.find? Env2.stateSubstInfo.subst fresh_name = none ∧
          (∀ a t, Maps.find? Env2.stateSubstInfo.subst a = some t →
            fresh_name ∉ LMonoTy.freeVars t) :=
    ...
    | .ite m c th el =>
      ...
      have h_ne2 : Env2.context.types ≠ [] := h_ctx2 ▸ h_ne'
      ...
      have h_ctx3 : Env3.context = Env.context := h_props3.context ▸ h_ctx2
...
theorem resolveAux_HasType :
  ...
  case h_app =>
    ...
    · -- Typing under arbitrary absorbing S
      ...
      have h_abs_S_Env1 : Subst.absorbs S Env1.stateSubstInfo.subst :=
        Subst.absorbs_trans Env1.stateSubstInfo.subst (Maps.remove substInfo.subst fresh_name) S
          h_abs_rem_Env1 h_abs_S
      have h_abs_S_Env2 : Subst.absorbs S Env2.stateSubstInfo.subst :=
        Subst.absorbs_trans Env2.stateSubstInfo.subst (Maps.remove substInfo.subst fresh_name) S
      ...
      have h_subst_e1t : LMonoTy.subst S (LMonoTy.subst substInfo.subst e1t.toLMonoTy) =
        ...
        exact LMonoTy.subst_absorbs S (Maps.remove substInfo.subst fresh_name) e1t.toLMonoTy h_abs_S
      have h_subst_e2t : LMonoTy.subst S (LMonoTy.subst substInfo.subst e2t.toLMonoTy) =
        ...
        exact LMonoTy.subst_absorbs S (Maps.remove substInfo.subst fresh_name) e2t.toLMonoTy h_abs_S
      -- Apply subst S to h_unify_eq: subst S e1t = arrow [subst S e2t, subst S (subst substInfo (ftvar fresh))]
  ...
  case h_abs =>
    ...
    · -- Typing under arbitrary absorbing S
      ...
      have h_xv_fresh_maps : Maps.find? Env.context.types xv = none := by
        ...
        suffices h_all_none : ∀ (types : Maps T.Identifier LTy),
            (∀ m, m ∈ types → Map.find? m xv = none) →
            Maps.find? types xv = none by
          exact h_all_none _ h_per_scope
        ...
        | some bty_val =>
          ...
          | .ok (_, Env_ic) =>
            ...
            exact ⟨Env_ic,
              ...
              h_env1.symm⟩
        | none =>
          simp; intro h_tbv; elim_err h_tbv
      ...
      have h_tabs := HasType.tabs (TContext.subst Env.context S) m pn (xv, some xty)
        ...
        (by intro h_mem
            ...
            have h_not_known : xv ∉ TContext.knownVars Env.context := by
              ...
              have h_not_in_go : ∀ (types : Maps T.Identifier LTy),
                  (∀ m, m ∈ types → Map.find? m xv = none) →
              ...
              exact h_not_in_go _ h_per_scope h_kv
            exact h_not_known h_in_ctx)
        (by simp [LTy.isMonoType, LTy.boundVars])
        (by simp [LTy.isMonoType, LTy.boundVars])
        (by rw [h_ctx_subst_bridge] at h_body_S
            exact h_body_S)
        (by cases bty with
    ...
    · -- Typing: quant result type is bool, subst S bool = bool
      ...
      have h_ctx_bridge : Env1.context =
        ...
        have h_types_eq : Env_mid.genEnv.context.types = Env.genEnv.context.types :=
          congrArg TContext.types h_mid_ctx
        have h_aliases_eq2 : Env_mid.genEnv.context.aliases = Env.genEnv.context.aliases :=
          congrArg TContext.aliases h_mid_ctx
        rw [h_types_eq, h_aliases_eq2]
        congr 1
        exact (Maps.insert_eq_addInNewest_fresh _ _ _ h_xv_fresh_maps).symm
      have h_ctx_subst_bridge : Env1.context.subst S =
          { Env.context.subst S with types :=
      ...
      have h_tquant := HasType.tquant (TContext.subst Env.context S) m qk pn tr
        ...
        (by rw [h_ctx_subst_bridge] at h_body_S
            exact h_body_S)
        (by rw [h_ctx_subst_bridge] at h_tr_S
            exact h_tr_S)
        (by cases bty with
            | none => exact Or.inl rfl
            | some bty_val =>
  ...
  case h_ite =>
    ...
    have h_ws_t : WellScoped t Env1.context := by
      rw [h_ctx1]; intro x hx; apply h_ws; simp only [LExpr.freeVars]
      exact List.mem_append_left _ (List.mem_append_right _ hx)
    have ⟨_, h_ty_t⟩ := h_ih_t h_ws_t
    have h_ws_e : WellScoped e Env2.context := by
      rw [h_ctx2]; intro x hx; apply h_ws; simp only [LExpr.freeVars]
...
private theorem ctx_closed_of_check (Env : TEnv T.IDMeta)
  ...
  have h_types_eq : Env.context.types = Env.genEnv.context.types := rfl
  rw [h_types_eq] at h_find
...
theorem resolve_HasType_core :
  ...
  | nil =>
    ...
    have h_idem : LMonoTy.subst Env'.stateSubstInfo.subst e_typed.toLMonoTy = e_typed.toLMonoTy := by
      rw [← h_typed, applySubstT_toLMonoTy]
      exact LMonoTy.subst_idempotent Env'.stateSubstInfo.subst Env'.stateSubstInfo.isWF (et.toLMonoTy)
    refine ⟨fun S h_abs h_wf h_pkf => ?_, h_idem, h_envwf'⟩
    rw [← h_typed, applySubstT_toLMonoTy, LMonoTy.subst_absorbs S Env'.stateSubstInfo.subst _ h_abs]
    ...
    have h_upd_subst_id : TContext.subst Env_upd.context S = Env_upd.context := by
    ...
    suffices h_ctx_empty : Env.context = { types := [], aliases := Env.context.aliases } by
      rw [h_ctx_empty]; exact h_result
    cases h_ctx : Env.context with
    | mk t a => grind
  | cons hd tl =>
    simp [Maps.isEmpty, h_types] at h
    elim_err h
    ...
    rw [LMonoTy.subst_absorbs S Env'.stateSubstInfo.subst _ h_abs]
    exact h_ht
...
theorem resolve_HasType :
  ...
  have h_hastype := h_ht Env'.stateSubstInfo.subst (Subst.absorbs_refl _ Env'.stateSubstInfo.isWF)
